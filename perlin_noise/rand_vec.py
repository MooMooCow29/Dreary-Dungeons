...
class RandVec(object):
    ...
    def dists_to(self, coordinates: List[float]) -> Tuple[float, ...]:
        return tuple(coor1 - coor2 for coor1, coor2 in zip(coordinates, self.coordinates))

    def weight_to(self, coordinates: List[float]) -> float:
        weighted_dists = [fade(1 - abs(dist)) for dist in self.dists_to(coordinates)]
        return product(weighted_dists)

    def get_weighted_val(self, coordinates: List[float]) -> float:
        weighted_dists = self.dists_to(coordinates)
        weighted_values = [fade(1 - abs(dist)) for dist in weighted_dists]
        return product(weighted_values) * dot(self.vec, weighted_dists)
