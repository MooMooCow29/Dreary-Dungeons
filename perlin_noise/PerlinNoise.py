...
class PerlinNoise(object):
    def __init__(self, octaves: float = 1, seed: Optional[int] = None):
        if octaves <= 0:
            raise ValueError('octaves expected to be a positive number')

        if seed is not None and (not isinstance(seed, int) or seed <= 0):
            raise ValueError('seed expected to be a positive integer number')
    ...
    def noise(self, coordinates: Union[int, float, Iterable]) -> float:
        ...
        coor_bounding_box = [(math.floor(coordinate), math.floor(coordinate + 1)) for coordinate in coordinates]
        return sum(
            self.get_from_cache_or_create_new(coors).get_weighted_val(coordinates)
            for coors in each_with_each(coor_bounding_box)
        )

    def get_from_cache_or_create_new(self, coors: Tuple[int, ...]) -> RandVec:
        if coors not in self.cache:
            self.cache[coors] = RandVec(coors, self.seed * hasher(coors))
        return self.cache[coors]
